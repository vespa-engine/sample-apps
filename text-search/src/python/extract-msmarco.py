...
def load_msmarco_queries(qrel):
    ...
    queries = {}
    with gzip.open(
        os.path.join(download_dir, "msmarco-doctrain-queries.tsv.gz"),
        "rt",
        encoding="utf8",
    ) as f:
...
def load_msmarco_document_lookup_table():
    ...
    docoffset = {}
    with gzip.open(
        os.path.join(download_dir, "msmarco-docs-lookup.tsv.gz"), "rt", encoding="utf8"
    ) as f:
        tsvreader = csv.reader(f, delimiter="\t")
...
def load_msmarco_qrels():
    ...
    qrel = {}
    with gzip.open(
        os.path.join(download_dir, "msmarco-doctrain-qrels.tsv.gz"),
        "rt",
        encoding="utf8",
    ) as f:
        tsvreader = csv.reader(f, delimiter="\t")
...
def getcontent(docid, f, docoffset):
    try:
        ...
        title = re.sub(
            r"(?<=[a-z\.])(?=[A-Z])", " ", title
        )  # Fix "corrosion resistanceInstall with" etc
...
def extract_documents(docoffset):
    ...
    with open(
        os.path.join(download_dir, "msmarco-docs.tsv"), encoding="utf8"
    ) as f, open(docs_file, "w", encoding="utf8") as out, open(
        offset_file, "w", encoding="utf8"
    ) as offset:
...
def extract_queries(qrel, query_strings, docoffset):
    ...
    with open(query_file, "w", encoding="utf8") as out:
        for qid in qrel.keys():
            relevant_documents = ",".join(
                [docid for docid in qrel[qid] if docid in docoffset]
            )
