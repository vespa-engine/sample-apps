...
vespa query 'yql=select * from doc where userQuery()'\
 'input.query(qt)=embed(moderncolbert, @query)' \
  'input.query(q)=embed(alibaba_gte_moderncolbert, @query)' \
 'query=exchanging information by sound'

vespa query 'yql=select * from doc where userQuery() or ({targetHits: 1000}nearestNeighbor(embedding_alibaba_gte_modernbert, q))'\
