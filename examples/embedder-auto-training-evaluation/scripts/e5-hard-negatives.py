...
def search(query, query_id):
    ...
    if response.ok:
        ...
        data = {"query_id": query_id, "query": query, "positives": {"doc_id": []},
                "negatives": {"doc_id": []}}

        positive_documents = qrels.get(query_id, [])
        data["positives"]["doc_id"].extend(positive_documents)
...
def main():
    ...
    dataset = ir_datasets.create_dataset(queries_tsv=args.queries, qrels_trec=args.qrels)

    global qrels
    qrels = {query_id: documents.keys() for query_id, documents in dataset.qrels_dict().items()}
    global train_data
    train_data = []
