...
def create_conditions(conditions_list):
    ...
    for condition_str in conditions_list:
        parts = condition_str.split(" ", 2)  # Split into max 3 parts to handle "contains" operator properly
        
        if len(parts) < 3:
            raise ValueError(f"Invalid condition format: '{condition_str}'. Expected format: 'field operator value'")
...
def VespaRetriever(state: SubgraphState):
    ...
    condition_uq = qb.userQuery(UserQuery)
    ...
    if Filters !=  ['None']:
        ...
        CondList = create_conditions(Filters)
        cond_opt = qb.all(*CondList)

        q = (
            qb.select(columns)
            .from_(schema)
            .where( 
            ( condition_ann | condition_uq )
            & condition_cat
            & cond_opt
                )
        )
        ...
        q = (
            qb.select(columns)
            .from_(schema)
            .where( ( condition_ann | condition_uq )
                    & condition_cat)
        )


    print(str(q))
    resp = vespa_app.query( yql=q,
                            ranking="hybrid",
                            body={"input.query(q_embedding)": f"embed({UserQuery})"},
                            hits=num_hits)
