...
def write_json_file(terms, file_path):
    feed_list = [
        {'put': f'id:term:term:g=0:{re.sub(r" ", "/", term)}', 'fields': data}
        for (term, data) in terms.items()
    ]
    with open(file_path, "w") as f:
        json.dump(feed_list, f, indent = 0)
...
def count_terms(doc, term_length = 1, common_words = set()):
    ...
    for win_len in range(1, term_length + 1):
        for i in range(len(doc)+1-win_len):
            if not any(map(lambda word: word in common_words, doc[i:i+win_len])):
...
def process_doc(doc_fields, term_length = 1, common_words = set()):
    if doc_fields['title'] and doc_fields['content']:
        title_counts = count_terms(clean_text(doc_fields['title']), term_length, common_words)
        content_counts = count_terms(clean_text(doc_fields['content']), term_length, common_words)
        return {
...
def process_docs(obj, term_length = 1, common_words = set()):
    doc_counts = [process_doc(doc['fields'], term_length, common_words) for doc in obj]
    terms = {}
    ...
    for term in terms.keys():
        terms[term]['docs'] = sum([1 if term in doc else 0 for doc in doc_counts])
...
def main():
    if len(sys.argv) == 3:
        ...
        write_json_file(process_docs(read_json_file(sys.argv[1]), int(sys.argv[3])), sys.argv[2])
    elif len(sys.argv) == 5:
        write_json_file(process_docs(read_json_file(sys.argv[1]), int(sys.argv[3]), read_common_words(sys.argv[4])), sys.argv[2])
    else:
        print('Wrong number of arguments: python3 term_frequensies.py infile.json outfile.json [number_of_words_in_term [common_words.txt]]')
